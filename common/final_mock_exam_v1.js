(() => {
  const basicBank=[
    {id:"c_rad",section:"円運動",prompt:"180°をラジアンで答えなさい。",answers:["π","pi"],display:"π"},
  ...
  const big2=[
    {id:"grav_symbol",title:"人工衛星の軌道速度",context:"地球の質量をM、人工衛星の質量をm、地球中心からの距離をr、万有引力定数をGとする。",questions:[
      {prompt:"人工衛星にはたらく万有引力Fを式で答えなさい。",answers:["GMm/r²","GMm/r^2"],display:"GMm/r²"},
      {prompt:"円軌道を回る人工衛星の速さvを式で答えなさい。",answers:["√(GM/r)","sqrt(GM/r)"],display:"√(GM/r)"}
    ]},
    {id:"cone_force",title:"円錐振り子の力",context:"質量mの小球が、鉛直方向から角度θだけ傾いた糸で水平な円運動をしている。糸の張力をTとする。",questions:[
      {prompt:"鉛直方向のつり合いを式で答えなさい。",answers:["Tcosθ=mg","Tcos(theta)=mg","Tcosθ＝mg"],display:"Tcosθ = mg"},
      {prompt:"水平方向の運動方程式を、円運動の半径rと速さvを使って答えなさい。",answers:["Tsinθ=mv²/r","Tsin(theta)=mv^2/r"],display:"Tsinθ = mv²/r"}
    ]},
    {id:"spring_circle",title:"円運動するばね",context:"質量mの物体が、自然長からx伸びたばねにつながれ、半径r、速さvで水平な円運動をしている。ばね定数をkとする。",questions:[
      {prompt:"ばねの弾性力の大きさを式で答えなさい。",answers:["kx"],display:"kx"},
      {prompt:"ばねの弾性力が向心力になる運動方程式を答えなさい。",answers:["kx=mv²/r","kx=mv^2/r"],display:"kx = mv²/r"}
    ]}
  ];
  const big3=[
    {id:"grav_numeric",title:"地表付近の人工衛星",context:"地球の半径を6.4×10⁶ m、地表の重力加速度を9.8 m/s²とし、人工衛星が地表すれすれを円運動すると考える。",questions:[
      {prompt:"万有引力が向心力になる式を、地球質量M、衛星質量m、半径R、速さvで答えなさい。",answers:["GMm/R²=mv²/R","GMm/R^2=mv^2/R"],display:"GMm/R² = mv²/R"},
      {prompt:"GM/R²=gを使い、軌道速度vをgとRで表しなさい。",answers:["√(gR)","sqrt(gR)"],display:"√(gR)"},
    ...
    {id:"cone_speed",title:"円錐振り子の速さ",context:"長さLの糸につながれた質量mの小球が、鉛直方向から角度θで円錐振り子の運動をしている。張力をT、円運動の半径をrとする。",questions:[
      {prompt:"円運動の半径rをLとθで表しなさい。",answers:["Lsinθ","Lsin(theta)"],display:"Lsinθ"},
      {prompt:"鉛直方向のつり合いから、張力Tを式で表しなさい。",answers:["mg/cosθ","mg/cos(theta)"],display:"mg/cosθ"},
      {prompt:"Tsinθ=mv²/rを使い、v²をg、L、θで表しなさい。",answers:["gLsinθtanθ","gLsin(theta)tan(theta)"],display:"gLsinθtanθ"}
    ]}
  ];
  ...
  const normalize=s=>String(s??"").trim().toLowerCase().replace(/[　\s]/g,"").replace(/＝/g,"=").replace(/−/g,"-").replace(/÷/g,"/").replace(/・/g,"").replace(/×/g,"*").replace(/π/g,"pi").replace(/θ/g,"theta").replace(/²/g,"^2").replace(/√/g,"sqrt").replace(/[{}]/g,m=>m==="{"?"(":")");
  function correct(q,value){if(q.number!==undefined){const n=Number(String(value).replace(/,/g,""));return Number.isFinite(n)&&Math.abs(n-q.number)<=q.tolerance;}return q.answers.some(a=>normalize(a)===normalize(value));}
