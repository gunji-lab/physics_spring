(() => {
  const CONFIG = window.SPRING_STAGE_CONFIG || {};
  const BANKS = window.SPRING_BANKS || {};
  ...
  *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Hiragino Sans","Yu Gothic",sans-serif;line-height:1.7}
  ...
  .choice{width:100%;text-align:left;border:1px solid #cbd5e1;background:#fff;color:var(--text);border-radius:14px;padding:14px 16px;font-size:1.06rem;font-weight:700;cursor:pointer}.choice:hover{border-color:var(--accent);box-shadow:0 0 0 3px rgba(37,99,235,.10)}
  .choice.selected{border-color:var(--accent);background:var(--accent-soft);box-shadow:0 0 0 3px rgba(37,99,235,.16)}.choice:disabled{cursor:default;opacity:1}.choice.correct{border-color:#86efac;background:#ecfdf5;color:#14532d}.choice.wrong{border-color:#fecaca;background:#fef2f2;color:#7f1d1d}
  ...
  .result{text-align:center;padding:24px 8px}.scorebig{font-size:2.25rem;font-weight:900;margin:8px 0}.pass{color:var(--ok);font-weight:900}.fail{color:var(--ng);font-weight:900}.note{margin-top:16px;padding:12px 14px;border-left:5px solid #93c5fd;background:var(--accent-soft);border-radius:10px;color:#475569;font-size:.95rem}.badge{display:inline-block;font-size:.85rem;padding:2px 8px;border-radius:99px;background:#dbeafe;color:#1e40af;margin-bottom:8px;font-weight:900}
  input{width:100%;padding:14px 16px;font-size:1.1rem;border:1px solid #cbd5e1;border-radius:12px;margin-top:12px}input:focus{outline:none;border-color:var(--accent);box-shadow:0 0 0 3px rgba(37,99,235,.15)}.status{font-size:.92rem;color:#6b7280;margin-top:10px}.answer-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:end}.unit{font-weight:900;color:var(--muted);padding-bottom:13px}
  ...
  <div class="wrap"><div class="card">
    <h1>${escapeHTML(CONFIG.title || "バネトレーナー")}</h1>
    <div class="lead">${escapeHTML(CONFIG.lead || "バネの基本を練習します。")}</div>
    <div id="startScreen"><div class="note">${CONFIG.note || "10問ランダム出題、70%以上でクリアです。"}</div><div class="note">大学Googleアカウントで確認済みです。</div><input id="studentId" type="hidden" autocomplete="off"><div class="buttons"><button id="startBtn">スタート</button></div></div>
  ...
  function withId(q,key,i){return {...q,id:q.id || `SPRING_${key}_${i+1}`}}
  function allProblems(){return ["stage1","stage2","stage3","stage4"].flatMap(key=>(BANKS[key]||[]).map((q,i)=>withId(q,key,i)))}
  ...
  function displayText(s){return escapeHTML(s).replace(/\^2/g,"<sup>2</sup>").replace(/m\/s\^2/g,"m/s<sup>2</sup>").replace(/x²/g,"x<sup>2</sup>").replace(/v²/g,"v<sup>2</sup>")}
  function sameAnswer(a,b){return String(a.answer)===String(b.answer)&&a.unit===b.unit}
  function makeChoices(q){let pool=baseBank().filter(x=>x.unit===q.unit&&!sameAnswer(x,q));if(pool.length<3)pool=allProblems().filter(x=>x.unit===q.unit&&!sameAnswer(x,q));const labels=[];const add=x=>{if(!labels.includes(x))labels.push(x)};add(answerLabel(q));shuffle(pool).forEach(x=>add(answerLabel(x)));let d=1;while(labels.length<4){const n=Number(q.answer);add(`${formatNumber(Number.isFinite(n)?n+d:d)} [${q.unit}]`);d++}return shuffle(labels.slice(0,4))}
  function buildQuiz(source=null){if(source)return shuffle(source).slice(0,total).map(q=>({...q,choices:isWritten?[]:makeChoices(q)}));if(isWritten){const keys=["stage1","stage2","stage3","stage4"];return shuffle(keys.flatMap((k,idx)=>shuffle((BANKS[k]||[]).map((q,i)=>withId(q,k,i))).slice(0,idx<2?3:2))).slice(0,total)}const bank=baseBank();if(CONFIG.groups)return shuffle(CONFIG.groups.flatMap(g=>shuffle(bank.filter(q=>q.type===g.type)).slice(0,g.count))).map(q=>({...q,choices:makeChoices(q)}));return shuffle(bank).slice(0,total).map(q=>({...q,choices:makeChoices(q)}))}
  function normalizeInput(s){return String(s ?? "").trim().replace(/[０-９]/g,ch=>String.fromCharCode(ch.charCodeAt(0)-0xFEE0)).replace(/[−ー―]/g,"-").replace(/，/g,".").replace(/×10\^?/g,"e").replace(/,/g,"").replace(/\s/g,"").replace(/\[[^\]]*\]/g,"").replace(/N\/m|m\/s|kg|N|J|m|s/g,"")}
  function numericCorrect(raw,ans){const match=normalizeInput(raw).match(/[-+]?(?:\d+\.?\d*|\.\d+)(?:e[-+]?\d+)?/i);if(!match)return false;const v=Number(match[0]);const a=Number(ans);const tol=Math.max(0.02,Math.abs(a)*0.015);return Number.isFinite(v)&&Math.abs(v-a)<=tol}
  ...
  function showQuestion(){answered=false;selectedAnswer="";selectedButton=null;const q=quiz[index];$("count").textContent=`${index+1} / ${quiz.length}`;$("score").textContent=`正解 ${score}`;$("bar").style.width=`${index/quiz.length*100}%`;$("question").innerHTML=`<span class="badge">${escapeHTML(q.stage || CONFIG.badge || "")}</span><br>${displayText(q.text)}`;$("feedback").className="feedback";$("feedback").style.display="none";$("answerBtn").disabled=true;$("nextBtn").disabled=true;$("choices").innerHTML="";$("writtenArea").innerHTML="";if(isWritten){$("writtenArea").innerHTML=`<div class="answer-row"><input id="answerInput" autocomplete="off" placeholder="数値を入力"><span class="unit">${escapeHTML(q.unit)}</span></div>`;const input=$("answerInput");input.addEventListener("input",()=>{$("answerBtn").disabled=!input.value.trim()});input.addEventListener("keydown",e=>{if(e.key==="Enter"&&!$("answerBtn").disabled)check()});setTimeout(()=>input.focus(),50)}else{q.choices.forEach((c,i)=>{const b=document.createElement("button");b.className="choice";b.dataset.answer=c;b.innerHTML=`${["①","②","③","④"][i]}　${displayText(c)}`;b.onclick=()=>selectChoice(c,b);$("choices").appendChild(b)})}TrainerLog.startQuestion()}
  ...
  function finish(){TrainerLog.finishSession();$("quizScreen").style.display="none";$("finishScreen").style.display="block";const rate=Math.round(score/quiz.length*100);$("finalScore").textContent=`${score} / ${quiz.length} 問正解（${rate}%）`;$("finalMessage").innerHTML=rate>=70?`<div class="pass">${CONFIG.passMessage || "クリア！バネの基本が整理できています。"}</div>`:`<div class="fail">${CONFIG.failMessage || "式と単位を確認して、もう一度挑戦しましょう。"}</div>`;TrainerLog.showElapsedInFinalMessage();$("reviewBtn").style.display=wrongList.length?"inline-block":"none";TrainerLog.sendResult({score,total:quiz.length})}
